...
// Phase 54B: Luxury chat composer — source verification (file + marker)
...
const main = () => {
  const report = { fileExists: false, markerPresent: false };

  if (fs.existsSync(COMPOSER_PATH)) {
    report.fileExists = true;
    const content = fs.readFileSync(COMPOSER_PATH, "utf8");
    report.markerPresent = content.includes(MARKER);
  }

  const pass = report.fileExists && report.markerPresent;
