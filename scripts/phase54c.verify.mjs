// scripts/phase54c.verify.mjs
// Phase 54C: Safe-area bottom nav + voice guardrails verification
  ...
  for (const [name, cfg] of Object.entries(checks)) {
    ...
    if (!fs.existsSync(cfg.path)) {
      ...
      if (!content.includes(cfg.mustContain)) {
        result.reason = `missing required: "${cfg.mustContain}"`;
        pass = false;
