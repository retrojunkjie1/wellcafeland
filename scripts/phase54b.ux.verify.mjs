...
const fetchGet=async (url,ms=4000) => {
  try{
    ...
    return {ok:false,status:"ERR",text:null,error:{name:e?.name||"Error",message:e?.message||String(e)}};
  }
};
...
const readIndex=() => {
  ...
  const hasThreadcue=html.includes('data-wc-threadcue')||html.includes("You don't have to explain.");
  return {ok:true,hasApp,hasEntry,hasThreadcue};
};
...
const main=async () => {
  ...
  if(!index.ok || !index.hasThreadcue){
    pass=false;
  }
  report.markers={hasApp:index.hasApp,hasEntry:index.hasEntry,hasThreadcue:index.hasThreadcue};
  ...
  if(chosenBase){
    ...
    for(const r of routes){
      ...
      if(r==="/chat"&&res.text){
        const hasThreadcue=(res.text.includes('data-wc-threadcue')||res.text.includes("You don't have to explain."));
        report.routes[r].hasThreadcue=hasThreadcue;
        if(!hasThreadcue)pass=false;
      }
    }
  }

  console.log("PHASE54B_UX_VERIFY");
